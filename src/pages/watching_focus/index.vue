<template>
  <div class="container" v-cloak>
    ...
    <div class="water-fall-ctn" :style="{height: (leftH >= rightH ? leftH : rightH) + 'px' }">
      <div
        class="item"
        hover-class="btn-scale-hover"
        v-for="(item, i) in list"
        :key="i"
        :class="[item.isLeft ? 'isleft' : 'isright', item.finish ? 'isShow' : '']"
        :style="{height: (item.imgHeight + 60) + 'px', top: item.top + 'px'}"
      >
        <div class="inner">
          <img
            :src="item.picUrl"
            ...
            @error="handleImgLoadErr($event, item)"
            alt=""
          >
          <div class="bott-info">
